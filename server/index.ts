  ...
  for (const m of raw) {
    if (
      ...
      if (role === "user" || role === "assistant" || role === "system") {
        out.push({ role, content: (m as ChatMessage).content });
      }
    }
  }
  return out;
...
app.post("/api/image", async (req, res) => {
  try {
    ...
    if (engine === "reve") {
      if (!aimlApiKey) {
        res
          ...
          .json({
            error:
              "AIMLAPI_KEY not configured (Reve via AI/ML API — see .env.example)",
          });
        return;
      }
    ...
    const allowedAspect = new Set<string>([
      "21:9",
      "16:9",
      "3:2",
      "4:3",
    ...
    if (
      ...
      input.aspect_ratio = aspect_ratio as NanoBananaProInput["aspect_ratio"];
    }
